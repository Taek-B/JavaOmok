...
public class ForbiddenStone {
	//board[y][x]..
		...
		//장목
		private static boolean jangmok() {
			int result = 0;
		...
		private static boolean fiveStone() {
			int result = 0;
		...
		static boolean main(int[][] board , int x , int y , boolean playerA) {
			...
			result = samsam();
			if(result) {
				Player.forbiddenStoneMessage = "삼삼 금수입니다.";
				return true;
			}
			
			result = sasa();
			if(result) {
				Player.forbiddenStoneMessage = "사사 금수입니다.";
				return true;
			}
			
			result = jangmok();
			if(result) {
				Player.forbiddenStoneMessage = "장목(6목이상) 금수입니다.";
				return true;
			}
